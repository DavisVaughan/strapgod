...
The goal of strapgod is to create _virtual groups_ on top of a `tibble` or `grouped_df` that function as resamples of the rows of the original data frame. You can then efficiently perform various dplyr operations on this `resampled_df`, like: `summarise()`, `do()`, `group_map()`, and more, to easily compute bootstrapped and resampled statistics.
...
You can feed a `resampled_df` into `summarise()` or `group_map()` to perform
