...
`dplyr 0.8` adds some more neat group-wise functionality, but it is a little
unstable right now with regards to `group_split(keep = FALSE)` which is used
by the default of `group_nest()` and `group_map()`.
