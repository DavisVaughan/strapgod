...
This makes it easy to compute bootstraped estimates of individual statistics, along with bootstrapped standard deviations around those estimates.
...
The virtual groups are stored in the `group_data()` metadata of the `"resampled_df"` object. Every grouped data frame has one of these, and they are used internally to power the `group_by()` system.

```{r}
group_data(iris_boot)
```

The `.bootstrap` column contains the unique values of the group, and the `.rows` column is a list column, where each element is an integer vector. That integer vector holds the rows that belong to that specific group. So, for `.bootstrap == 1`, there is an integer vector with the 150 rows belonging to that resample.

```{r}
group_data(iris_boot)$.rows[[1]]
```

When a call to `collect()` is made, this row index information is used to construct the output. Essentially, we start with the `group_data()` and utilize the `.rows` info to replicate the rows of the original data frame for each group, building up the complete resampled data frame.
